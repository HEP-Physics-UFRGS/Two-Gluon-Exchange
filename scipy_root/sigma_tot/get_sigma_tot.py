...
start_sqrt_s = 100
  ...
  # Global parameter controlling energy scale
...
def main():
    ...
    for i, (mass_model, ensemble) in enumerate(scenarios):
        ...
        fig.add_trace(go.Scatter(
            x=sqrt_s_lst.copy(),
            y=sigma_tot_lst.copy(),
            mode='lines+markers',
            name=f'{mass_model} {ensemble}',
            line=dict(dash=dash_styles[i], width=2),
            marker=dict(symbol='circle')
        ))

    fig.update_layout(
        title='Sigma Tot vs. sqrt(s) all models',
        xaxis_title='sqrt(s) [GeV]',
        yaxis_title='Sigma Tot [mb]',
        yaxis_range=[80, 120],
        xaxis_type='log',
        xaxis_range=[np.log10(2000), np.log10(13000)],
        legend_title='Modelo'
    )

    # output_dir = "sigma_tot/plots"
    # os.makedirs(output_dir, exist_ok=True)
    # fig.write_html(f"{output_dir}/cross_section_plot_all_models.html")
    # fig.write_image(f"{output_dir}/cross_section_plot_all_models.pdf")
